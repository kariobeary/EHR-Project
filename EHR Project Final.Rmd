...
prescription$pres <- prescription$DRUG_NAME
prescription$pres <- ifelse(is.na(prescription$pres), 0, 1)
...
hospitalization <- full_join(diagnostic, hosp, by="PAT_ID")
...
m1 <- lmer(HOSP_DUR~PAT_DPT_GRP+(1|PAT_ID), hospitalization, REML=0)
m1
summary(m1)

# random intercept + random slope
m2 <- lmer(HOSP_DUR~PAT_DPT_GRP+(PAT_DPT_GRP|PAT_ID), hospitalization, REML=0)
m2
summary(m2)
...
m3 <- lmer(HOSP_DUR~PAT_DPT_GRP+(1|PAT_ID)+(0+PAT_DPT_GRP|PAT_ID), hospitalization, REML=0)
m3
summary(m3)
...
# From the result, it seems there is very little difference between m2 and m3. 

# Therefore, to answer the study question, add number of hospitalization to the equation
m4 <- lmer(HOSP_DUR~PAT_DPT_GRP+NUM_HOSP+(1|PAT_ID)+(0+PAT_DPT_GRP|PAT_ID), hospitalization, REML=0)
m4
summary(m4)
```
...
m5 <- lmer(HOSP_DUR~PAT_DPT_GRP+NUM_HOSP+PAT_AGE+PAT_SEX_COD+(1|PAT_ID)+(0+PAT_DPT_GRP|PAT_ID), hospitalization, REML=0)
m5
summary(m5)

# with only age and sex as confounders -- Q: I wonder if this is necessary and whether it answers our question
m6 <- lmer(HOSP_DUR~PAT_DPT_GRP+PAT_AGE+PAT_SEX_COD+(1|PAT_ID)+(0+PAT_DPT_GRP|PAT_ID), hospitalization, REML=0)
m6
summary(m6)
